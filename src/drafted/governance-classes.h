 ...
 *
 *   // network
 *   CBitNexusNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.bitnexus.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CBitNexusNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CBitNexusNetwork(UniValue objIn)
    {
...
// // can be under: BitNexusNetwork
// //   -- signature requirements : Key1(User)
// class CBitNexusNetworkVariable : public CGovernanceObject
// {
// private:
