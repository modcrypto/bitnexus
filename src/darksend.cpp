...
{
    if(fLiteMode) return; // ignore all BitNexus related functionality
...
/*
    ...
    allow endless transaction that would bloat BitNexus and make it unusable. To
...
bool CDarkSendSigner::IsVinAssociatedWithPubkey(const CTxIn& txin, const CPubKey& pubkey)
{
    ...
    if(GetTransaction(txin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if(mnodeman.IsValidCollateral(out.nValue) && out.scriptPubKey == payee) return true; 
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("bitnexus-privatesend");
